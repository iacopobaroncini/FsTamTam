...
public class FsTamTam {
    ...
    synchronized public static void trace(String text, double d) {
        if (state.ordinal() < State.CONNECTED.ordinal()) return;
        send(String.format("T%s = %f", text, d));
    }

    synchronized public static void trace(String text, int i) {
        if (state.ordinal() < State.CONNECTED.ordinal()) return;
        send(String.format("T%s = %d", text, i));
    }
    ...
    synchronized public static void error(String errorMessage, double d) {
        if (state.ordinal() < State.CONNECTED.ordinal()) return;
        send(String.format("E%s = %f", errorMessage, d));
    }

    synchronized public static void error(String errorMessage, int i) {
        if (state.ordinal() < State.CONNECTED.ordinal()) return;
        send(String.format("E%s = %d", errorMessage, i));
    }
